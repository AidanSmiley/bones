...
void Renderer::load_models()
{
	...
	//lets load and make the other cubes
	Object cubeY_object("./objs/cube.obj");
	cubeY_object.obj_color = glm::vec4(1.0, 1.0, 0.0, 1.0);
	cubeY_object.obj_name = "yellow_cube";

	Object cubeP_object("./objs/cube.obj");
	cubeP_object.obj_color = glm::vec4(1.0, 1.0, 0.0, 1.0);
	cubeP_object.obj_name = "pink_cube";

	Object cubeB_object("./objs/cube.obj");
	cubeB_object.obj_color = glm::vec4(1.0, 1.0, 0.0, 1.0);
	cubeB_object.obj_name = "blue_cube";
	...
	bind_vaovbo(cubeY_object);
	bind_vaovbo(cubeP_object);
	bind_vaovbo(cubeB_object);
	bind_vaovbo(plane_object);
	bind_vaovbo(arrow_object);
	
	// Here we only load one model
	obj_list.push_back(cubeB_object);
	obj_list.push_back(cubeP_object);
	obj_list.push_back(cubeY_object);
...
void Renderer::draw_bones(Shader& shader, Bone_Animation* m_bone_animation)
{
	...
	Object* yellow_bone = nullptr;
	Object* pink_bone = nullptr;
	Object* blue_bone = nullptr;
	...
	for (unsigned int i = 0; i < obj_list.size(); i++)
	{
		...
		if (obj_list[i].obj_name == "yellow_cube") {
			yellow_bone = &obj_list[i];
		}
		if (obj_list[i].obj_name == "pink_cube") {
			pink_bone = &obj_list[i];
		}
		if (obj_list[i].obj_name == "blue_cube") {
			blue_bone = &obj_list[i];
		}

	}
	if (bone_obj == nullptr || yellow_bone == nullptr || pink_bone == nullptr || blue_bone == nullptr)
		return;
	...
	// yellow bone
	glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1, GL_FALSE, glm::value_ptr(m_bone_animation->yellowMat));
	yellow_bone->obj_color = m_bone_animation->colors[1];
	draw_object(shader, *yellow_bone);

	// pink bone
	glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1, GL_FALSE, glm::value_ptr(m_bone_animation->pinkMat));
	pink_bone->obj_color = m_bone_animation->colors[2];
	draw_object(shader, *pink_bone);

	// blue bone
	glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1, GL_FALSE, glm::value_ptr(m_bone_animation->blueMat));
	blue_bone->obj_color = m_bone_animation->colors[3];
	draw_object(shader, *blue_bone);
